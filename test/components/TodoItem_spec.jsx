...
describe('TodoItem', () => {
    ...
    it('strikes through the item if it is completed', () => {
        const text = 'React';
        const component = renderIntoDocument(
          <TodoItem text={text} isCompleted={true}></TodoItem>
        );
        const todo = scryRenderedDOMComponentsWithTag(component, 'li');
    ...
    it('should look different when editing', () => {
        const text = 'React';
        const component = renderIntoDocument(
          <TodoItem text={text} isEditing={true}></TodoItem>
        );
        const todo = scryRenderedDOMComponentsWithTag(component, 'li');
    ...
    it('invokes callback when the delete button is clicked', () => {
        const text = 'React';
        var deleted = false;
        // We defined a mock deleteItem function
        const deleteItem = () => deleted = true;
        const component = renderIntoDocument(
          <TodoItem text={text} deleteItem={deleteItem}></TodoItem>
        )
